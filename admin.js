...
const metricReferralValue = document.getElementById("metric-referral-value");
...
const refreshReferralsBtn = document.getElementById("refresh-referrals-btn");
...
function renderMetrics() {
  ...
  if (metricReferralValue) {
    metricReferralValue.textContent = formatMoney(
      currentReferrals.reduce((sum, item) => sum + Number(item.bonus_amount || 0), 0)
    );
  }
}
...
function renderUsersTable(users) {
  if (!users.length) {
    usersTbody.innerHTML = `<tr><td colspan="9"><div class="empty-state">No users found</div></td></tr>`;
    return;
  }
    ...
    .map(
      (user) => `
        <tr>
          ...
          <td>
            <div class="cell-stack">
              <strong>${Number(user.referral_count || 0)}</strong>
              <span class="muted">${escapeHtml(user.referral_code || "-")}</span>
              <span class="money-cell">${formatMoney(user.referral_bonus_total || 0)}</span>
            </div>
          </td>
...
function renderReferralRows(items) {
  ...
  if (!items.length) {
    referralsTbody.innerHTML = `<tr><td colspan="6"><div class="empty-state">No referrals found</div></td></tr>`;
    return;
  }

  referralsTbody.innerHTML = items
    .map(
      (item) => `
        <tr>
          <td>
            <div class="cell-stack">
              <strong>${escapeHtml(item.referrer_name || "-")}</strong>
              <span>@${escapeHtml(item.referrer_username || "-")}</span>
              <span class="muted">${escapeHtml(item.referrer_email || "-")}</span>
            </div>
          </td>
          <td>
            <div class="cell-stack">
              <strong>${escapeHtml(item.referred_name || "-")}</strong>
              <span>@${escapeHtml(item.referred_username || "-")}</span>
              <span class="muted">${escapeHtml(item.referred_email || "-")}</span>
            </div>
          </td>
          <td class="money-cell">${formatMoney(item.bonus_amount || 0)}</td>
...
async function loadReferrals() {
  await ensureSupabase();
  currentReferrals = await adminGetReferrals(null);
  renderReferralRows(currentReferrals);
  renderMetrics();
}
...
async function loadDashboard() {
  try {
    ...
    await Promise.all([loadUsers(), loadRequests(), loadReferrals()]);
...
refreshReferralsBtn?.addEventListener("click", async () => {
  await loadReferrals();
  displayStatus("Referral records refreshed.", "success");
});
