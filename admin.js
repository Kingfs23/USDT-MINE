  ...
  usersTbody.innerHTML = users
    ...
    <tr>
      ...
      <td class="action-buttons">
        <button class="action-btn btn-edit" onclick="editUserBalance('${user.id}', '${user.user_id_display}', '${getSafeName(user).replace(/'/g, "\\'")}')">Edit</button>
        <button class="action-btn btn-delete" onclick="deleteUserAccount('${user.id}', '${user.user_id_display}', '${getSafeName(user).replace(/'/g, "\\'")}')">Delete</button>
      </td>
    </tr>
  `
    )
...
async function loadAllUsers() {
  try {
    if (!isSupabaseConfigured || !supabase) {
      throw new Error("Supabase is not available on this page. Check supabase-config.js and refresh.");
    }
