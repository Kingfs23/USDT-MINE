  ...
  return `
    <article class="transaction-item">
      <div class="notification-main">
        ...
        <h3>Deposit</h3>
        <p>${transaction.network || "Bank Transfer"}</p>
      </div>
...
function renderDepositHistory() {
  if (!depositHistory.length) {
    depositHistoryList.innerHTML = `<p class="empty-state">No notifications.</p>`;
    return;
  }
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const draft = {
      amount,
      note: inputValue("deposit-note"),
      senderBankName: inputValue("sender-bank-name"),
      senderAccountNumber: inputValue("sender-account-number"),
      senderAccountName: inputValue("sender-account-name"),
...
window.setTimeout(() => {
  loadDepositHistory().catch((error) => {
    depositHistoryList.innerHTML = `<p class="empty-state">${error.message || "Unavailable."}</p>`;
  });
}, 0);
