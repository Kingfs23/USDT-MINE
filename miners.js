...
async function renderPlans() {
  ...
  grid.innerHTML = MINER_PLANS.map((plan) => planCardMarkup(plan, Number(profile.account_balance || 0))).join("");

  const activeCount = purchases.filter((purchase) => purchase.status === "active").length;
  setStatus(
    statusElement,
    `Wallet balance: ${formatMoney(profile.account_balance || 0)} | Active miners: ${activeCount} | Every miner credits daily profit for 30 days.`,
    "default"
  );
