...
async function renderWallet() {
  ...
  walletStatus.textContent = `Current wallet balance: ${formatMoney(
    profile.account_balance || 0
  )} | Conversion rate: NGN ${CONVERSION_RATE.toLocaleString("en-US")} per $1`;
...
withdrawForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (amount > Number(profile.account_balance || 0)) {
      throw new Error("Withdrawal amount cannot be more than your current wallet balance.");
    }
