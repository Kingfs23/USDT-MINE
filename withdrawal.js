...
function renderWithdrawHistory() {
  if (!withdrawHistory.length) {
    withdrawHistoryList.innerHTML = `<p class="empty-state">No notifications.</p>`;
    return;
  }
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    await showBrowserNotification("Withdrawal requested", "Your withdrawal request was sent for admin approval.");
...
window.setTimeout(() => {
  loadWithdrawHistory().catch((error) => {
    withdrawHistoryList.innerHTML = `<p class="empty-state">${error.message || "Unavailable."}</p>`;
  });
}, 0);
